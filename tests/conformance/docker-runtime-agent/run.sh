...
compose_file=${COMPOSE_FILE:-tests/conformance/docker-runtime-agent/compose.yml}
compose="docker compose --profile conformance -f $compose_file"
evidence_file=${CONFORMANCE_EVIDENCE:-docker-agent-conformance.log}
prepare_evidence=$(mktemp "${TMPDIR:-/tmp}/graft-docker-runtime-prepare.XXXXXX")
bootstrap_evidence=$(mktemp "${TMPDIR:-/tmp}/graft-docker-runtime-bootstrap.XXXXXX")
...
run_driver \
  --phase prepare \
  --agent-id docker-runtime-agent \
  --image-digest "$GRAFT_CONFORMANCE_AGENT_IMAGE_DIGEST" \
  --agent-version "${GRAFT_CONFORMANCE_AGENT_VERSION:-fixture}" \
  --enrollment-ref docker-runtime-agent-conformance \
  --automation-id docker-runtime-agent-conformance \
...
$compose run --rm --no-deps docker-runtime-agent
...
umask 077
$compose logs --no-color postgres vault vault-init migrate backend docker-runtime-agent > "$evidence_file"
chmod 0600 "$evidence_file"
echo "Docker Runtime Agent conformance completed. Unredacted fixture service logs are in $evidence_file; treat them as secret material and do not commit them."
