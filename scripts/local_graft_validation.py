...
def render_readme(local_root: Path) -> str:
    ...
    return "\n".join((
        ...
        *rows,
        "",
        "## Credentials (local operator record)",
        "",
        "Replace the placeholders below after setting the credentials in each instance:",
        "",
        "| Instance | Username | Password |",
        "| --- | --- | --- |",
        "| `beta` | `<REPLACE_BETA_USERNAME>` | `<REPLACE_BETA_PASSWORD>` |",
        "| `latest` | `<REPLACE_LATEST_USERNAME>` | `<REPLACE_LATEST_PASSWORD>` |",
        "| `fixed` | `<REPLACE_FIXED_USERNAME>` | `<REPLACE_FIXED_PASSWORD>` |",
        "",
        "The initial login is `graft` / `graft-admin`; change it after first login and update the local record above.",
        "Credential values are local operator records only and are not read or reset by this tool.",
        "",
