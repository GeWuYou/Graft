...
- `GRAFT_UPDATE_COMPOSE_ROOT` 非空时拥有最高优先级：它必须是宿主机绝对路径，并以相同绝对路径挂载进 runner；该路径校验失败时不得回退到自动发现。为空时，server 通过 Docker API 检查当前 server 容器的 Compose labels、config files 和 bind mounts，生成候选并要求管理员确认后才能执行。
