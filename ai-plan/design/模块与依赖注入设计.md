...
- 本文中的 `Plugin` 等少量导出符号属于历史命名；当前 canonical 物理路径已经迁到 `module` / `modules`，跨模块稳定边界也已经迁到 `internal/moduleapi`
...
- 当前以 `module.Spec`、`module.Builder` 与 compile-time generated registry 作为 module registry，保留的 `Plugin` 导出命名只用于兼容现有生命周期接口语义，减少多工作树共享接线冲突
