...
- i18n governance 通过 `bun run lint:i18n` 收口到 `bun run check`，因此 CI 继续执行 `bun run check` 即可覆盖 i18n；本地 hooks 只在 `web/src/**`、`web/scripts/check-i18n-governance.ts` 或 `web/package.json` 变化时增量触发 `lint:i18n`
