...
* `app/` 放壳层页面入口，例如登录、结果页、首页入口和其他不归单个业务模块长期拥有的页面
* `modules/` 放插件对应的前端模块
* `shared/` 只放跨模块可复用且无业务语义的页面片段、基础组件或通用组合逻辑
* `components/` 放通用组件，不放业务耦合逻辑
* `store/` 只放跨页面共享状态
* `router/` 处理静态路由和动态路由装配
* `contracts/` 放平台级前端稳定契约
* `locales/` 处理应用级 locale 状态、消息目录和格式化入口

页面退休迁移的长期运行面约束：

* `app/**` 是壳层页面真值，不承担业务模块实现
* `web/src/modules/<name>/**` 是业务模块页面、API、局部类型、模块私有契约和模块消息源的长期真值
* 新模块默认目录为 `pages`、`components`、`api`、`contract`、`types`、`locales`
* `modules/<name>/bootstrap-routes.ts` 是模块唯一允许对外导出的动态路由入口
* 永远不要重新引入根级 `web/src/pages/**` 作为长期运行面
* 本次迁移切片只收口页面归属和导入边界，不主动发起新的 `shared/` 抽取

多 worktree 准备阶段的 `web` 目录所有权补充约束：

* `layouts/`、`router/`、`locales/`、全局 `store`、平台级 `contracts/` 和 `app/` 属于 `shell-owned` 目录
* `web/src/modules/<name>/` 属于模块自拥有边界，模块内部应承载该功能的页面、API、局部类型、模块私有稳定契约与模块消息源
* 后续 long-lived worktree 的默认 `web` owned scope 按模块粒度冻结，不按 `api/`、`app/`、`store/` 等技术层切分
* `shell-owned` 目录不归单个 feature worktree 长期占用，只允许通过集中集成切片修改

目录边界补充约束：

* `app/**` 只允许依赖 `shared/**`、`contracts/**`、`router/**`、`store/**` 与 `modules/*/contract/**`
* `app/**` 不得导入 `modules/*/api`、模块 `pages`、模块 `components` 或模块局部 composables
* `modules/**` 不得反向导入 `app/**`
...
* 当前菜单本地化 contract 优先消费后端返回的 `title_key`
* `title` 作为当前阶段兼容回退文案保留，但不得与 `title_key` 演变为两套长期并行真值
* 在 `title_key` 缺失时，`web` 才回退到 `title`
...
* 已标记 `deprecated` 的前端 contract 不得在新代码中继续作为默认真值使用；兼容期 alias 只能留在迁移桥接层
...
如果旧代码还未完全迁入模块目录，至少也要保证“模块接入壳层”的注册点在 `modules/` 下，而不是继续把共享壳层文件当作模块实现目录。
