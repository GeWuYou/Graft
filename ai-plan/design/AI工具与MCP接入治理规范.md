...
- `eff-u-code` / `fuck-u-code`
  ...
  - 接入方式：开发者本机使用 Bun 安装；当前验证通过的本机安装方式：

    ```bash
    bun add -g eff-u-code
    ```

  - 仓库入口：通过 `python3 scripts/run_eff_u_code.py` 调用本地 `bunx --bun --package eff-u-code fuck-u-code analyze ...`
  - 约束：
    ...
    - 不得加入 `server/go.mod`、`web/package.json`、根级 package manifest、runtime 脚本或部署流程。
...
- `graft-sql-migration` 是创建或修改 live Atlas/PostgreSQL migration SQL 的专用治理 skill；SQL 注释硬规则、
  ...
  `ai-plan/design/数据库表设计与迁移规范.md` 为准。
