...
- `server/plugins/*`、`internal/plugin*` 仍是历史命名；当前 live 物理路径已迁到 `server/modules/*` 与 `internal/module*`
...
* 首阶段不要求把全部页面与历史插件文案迁移为多语言资源，但新公共错误响应、模块消息注册与后续模块接入应优先使用稳定 key
...
5. 初始化 HTTP 服务
6. 初始化核心注册中心与服务容器
7. 注册模块（历史 plugin 注册入口）
8. 校验模块依赖并排序
9. 执行模块 `Register`（历史 plugin 生命周期命名）
10. 冻结 i18n 等声明式注册面
11. 执行模块 `Boot`（历史 plugin 生命周期命名）
12. 启动 HTTP 服务
...
## 6. 模块系统设计（历史 plugin 命名）

### 6.1 模块接口与 compile-time authority（历史 plugin 命名）
...
模块名与模块依赖的 canonical authority 来自 compile-time `ModuleSpec`，而不是运行时插件实例再维护第二份 `Name()` /
