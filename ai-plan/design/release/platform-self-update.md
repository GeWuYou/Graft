...
管理员可调用 `POST /api/platform/updates/operations/{operationID}/recovery` 启动一次性恢复 runner。该操作要求 `platform-update.manage`，且只接受 runner identity 匹配且已经退出、snapshot 已验证为 non-terminal、并且 migration 尚未开始的操作；运行中、已终态、身份不匹配、已恢复、state 不可读或 migration 已开始均 fail closed。恢复容器使用部署时显式配置的 `GRAFT_UPDATE_RECOVERY_RUNNER_IMAGE`，该值必须是官方 `graft-compose-runner` 的 immutable SHA-256 reference；不得复用可能尚未支持恢复协议的故障 runner 镜像，缺失或无效配置必须明确拒绝恢复。恢复 runner 只写入安全 terminal failure/rollback 结果，绝不继续被中断的升级；terminal projection 完成后才允许新升级重新进行资格检查。
