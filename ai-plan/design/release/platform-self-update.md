...
`release-manifest.json.sha256` 与 manifest 同时作为 GitHub Release asset 发布；读取端先验证 manifest checksum，后验证 JSON 内容。Compose runner 从 `server/runner/compose/Dockerfile` 由同一 release workflow 构建和推送；发布 workflow 在 Runner 输入未变化时，可以在 checksum、schema、channel、官方 image identity、GHCR manifest 类型与平台校验通过后复用最近有效同频道 Release 的 immutable digest。复用时，manifest 先绑定该 digest，当前 release 的 runner tag 只在 GitHub Release 创建成功后建立，避免失败清理把共享的历史 package version 一并删除。`minimum_source_version` 是可选 SemVer 约束；出现时读取端必须验证并执行它，发布流程在具备明确兼容策略前不生成该字段。manifest 绝不接受仓库变量、外部断言 digest 或 mutable tag。`required_before_runtime` 表示每个受支持升级都必须执行显式且幂等的 migration command；它不声称每个 release 都包含 SQL 变更。目标 manifest 的 tag、SemVer channel、Release Notes URL、升级说明、artifact 名称与逐项 SHA256、server/web/runner digest/reference 必须交叉校验，任何不一致、缺失 runner 或 mutable runner identity 都使目标不可执行。

正式 GitHub Release 创建前的任一构建、smoke 或镜像发布失败都会进入失败清理路径。清理只在同 tag 的 GitHub Release 不存在、且远端 tag 在每次删除前仍指向本次 workflow 的触发 commit 时执行：它删除 server、web 和非复用 runner 中只带本次 release-owned tag 集合的 GHCR package version。stable release 的集合为 `vMAJOR.MINOR.PATCH` 与 `MAJOR.MINOR.PATCH`，Beta release 的集合仅为 `vMAJOR.MINOR.PATCH-beta.N`，随后清理 Git tag。若 GHCR API 失败、tag 缺失或移动、或 package version 的 tag 集合超出预期，清理必须失败关闭，绝不删除共享历史版本。GitHub Release 已创建后不自动删除 tag 或 package version，也不承诺数据库 schema 的事务性回滚。
...
更新目录的候选状态是 `AVAILABLE`。用户确认后，server 持久化受授权的 update request，但该记录不是执行状态或 lease table。`graft-compose-runner` 随后取得一个 operation lease，成为该 operation 的唯一 lifecycle controller，并在官方 Compose 的 named update-state volume 中持久化版本化原子 `current.json` snapshot 和 append-only event records。cutover 完成后，runner 是 schema-v2 runtime 的唯一 writer；server 只读挂载并在 schema、operation binding、revision、lease 与完整性校验成功后消费状态。运行时只接受 schema-v2 snapshot；schema-v1 不再进入 normal server/runner 解析路径。schema v2 的每个 non-terminal snapshot 包含 `lease_epoch`、`lease_heartbeat_at` 和 `lease_expires_at`：初始化成功先写入 `READY` 与 lease，独立 heartbeat 每 30 秒原子续租一次，过期时间为五分钟。heartbeat 仅更新 `current.json` 与 snapshot revision，不追加 event 或 UI phase。状态还包含 `operation`、`phase`、`progress`、safe `message`、`started_at`、`finished_at`、safe structured `error`、`runner_id` 和绑定目标/冻结 deployment snapshot 的版本化 identity；不包含凭证、host path、backup location/content、命令输出或 Docker stderr。事件记录按 operation 隔离，只有单调 revision、时间、phase、allowlisted action code 与安全文案输入；它们提供可回放的节点时间线，不是 runner stdout/stderr 的代理。

执行 phase 固定为 `READY -> PREFLIGHT -> BACKUP -> PULL_IMAGES -> STOP_SERVICES -> APPLY_UPDATE -> MIGRATION -> START_SERVICES -> HEALTH_CHECK -> SUCCESS`；任一阶段可至 `FAILED`，而迁移前的受控配置/镜像恢复以 `ROLLBACK` 终止。runner 在每个不可逆动作前先持久化 transition。迁移开始后不执行自动数据库 rollback/restore，失败只能记录 `FAILED` 与人工恢复证据。phase、heartbeat 和 terminal write 共用一个 state-store mutex/fencing 边界：只有匹配 `runner_id + lease_epoch` 的未过期持有者可写入；过期持有者不能续租或写回。recovery runner 仅能在 lease 过期后取得递增 epoch，因此旧 runner 即使恢复执行也不能覆盖 recovery terminal result。
