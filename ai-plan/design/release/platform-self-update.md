...
server 每分钟 reconcile state volume，并在 API 查询时直接计算 lease。v2 非终态 snapshot 的 `lease_expires_at` 已过期时，server 必须返回 `state_source=runner_lost`、`state_available=false`、`error=PLATFORM_UPDATE_RUNNER_LOST`，并只保留最后可信的 `phase`、`progress` 与 safe `message` 作为诊断上下文；该投影不是运行中的 `READY` 或其他 phase。若首次 snapshot 尚未写出，数据库授权记录创建五分钟后也投影为 `runner_lost`。schema-v1 snapshot 保留 30 分钟兼容桥（15 分钟执行上限加 15 分钟宽限），超过后同样失联。`runner_terminated` 保留为旧客户端消费兼容，直到最低支持升级来源均能写出 schema-v2 lease snapshot，但 server 不再以 Docker container inventory、exit code、container logs 或容器是否仍存在决定状态或 recovery 前置条件。浏览器必须停止轮询，并为持有 `platform-update.manage` 的管理员显示受控诊断入口；server 只持久化一条 allowlisted 诊断，绝不将 Docker stderr、命令、host path、凭证或 state-store 原始 I/O 错误返回给 API/UI。
