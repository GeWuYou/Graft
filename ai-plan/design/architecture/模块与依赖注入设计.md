...
- Phase 1 使用有界内存 buffer、worker pool、批量消费、有界重试和优雅关闭，交付语义为 best-effort；队列饱和和关闭超时必须显式反馈或记录，不能静默丢弃
- 可靠投递需求进入后续 Phase 2：业务状态变更与 outbox 写入必须在同一数据库事务内，消费者按 `(event_id, consumer_id)` 保持幂等并接受至少一次投递；不得由业务模块自行 dual-write 数据库与外部 MQ
