...
- 长生命周期 server feature worktree 默认只长期拥有 `server/modules/<name>/**`
...
- `internal/moduleregistry/generated.go` 当前保持 tracked；长生命周期 feature worktree 不直接修改它，相关更新应通过短生命周期 integration / core slice 协调合入
...
- 若某个方向仍需要频繁同时修改 module-owned 代码和 core wiring / core schema，说明它还处在共享基线治理阶段，尚不适合独立长期 worktree
