...
- `platform-network` 是平台出站网络策略的唯一 owner。它通过 `OutboundNetworkProvider` 暴露当前有效策略，并通过
  `OutboundHTTPClientFactory` 暴露 `NewClient(opts ...OutboundHTTPClientOption)`；消费者只能使用该 factory，不能自行
...
- 网络诊断通过可注册的 `OutboundNetworkDiagnosticTarget` capability 暴露：target 自己声明稳定 `Name`、本地化
  `DisplayName` 并执行受控 `Execute(ctx)`。诊断路由只能选择已注册 target，不能接收管理员提供的 URL；首个 target 是
