...
* `server/internal/realtime`
  负责统一 topic、发布订阅 hub、WebSocket gateway、ticket 消费和连接生命周期
* `server/modules/<name>/...`
  负责把模块自有事件、日志或资源快照发布到 canonical topic
* `web`
  先通过普通 HTTP 鉴权接口申请单次 realtime ticket，再通过统一 `/ws` gateway 订阅 topic
...
* 模块不得再各自发明第二套长期 WebSocket 入口用于普通订阅式事件推送
