...
Builder Agent 与 Runtime Agent 拥有独立配置、身份、传输和发布生命周期。Agent 不直接拥有业务数据库真相，
不绕过 Server 的授权、Task/Submission、审计和 Provider conformance 边界。

### 5.2 Runner

Runner 只执行受控的外部副作用，例如 Compose、Helm 或 Terraform。它的输入应是不可变 operation snapshot，
...
4. 迁移独立 Agent、部署拓扑与 conformance fixture。
