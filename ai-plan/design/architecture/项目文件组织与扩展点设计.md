...
Docker 能力采用一个独立部署的 `docker-runtime-agent`。现有实验期 `docker-builder-agent` 直接迁移并改名，
...
4. 将实验期 Docker Builder Agent 直接迁移为单一 Docker Runtime Agent，迁移部署拓扑与 conformance fixture。
