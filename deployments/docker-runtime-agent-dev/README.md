# Docker Runtime Agent Local Dependencies

This development-only topology starts isolated Redis and Vault for a
host-debugged Docker Runtime Agent. It supports two explicit Server database
modes and uses only the ignored `.data/docker-runtime-agent-dev` directory for
Agent-local state:
...
`graft dev docker-runtime-agent prepare` defaults to `shared`: it reads
`server/.env`, writes the Server-owned `server/.env.docker-runtime-agent` by
...
separately. Use
`graft dev docker-runtime-agent prepare --database-mode isolated` for a
...
`server/agents/docker-runtime-agent/agent.json`. It is generated, ignored, and
...
discovery/summary, Container snapshot/resource/log/stream/event/stats/
interactive reads, Deployment Runtime's current-server Docker facts, and
bounded Update observation/recovery. Update Controller launch, Build, and
finite Container/Application mutations are Agent-owned; the Agent remains the
only always-on process that performs Docker side effects.
...
`.data/docker-runtime-agent-dev` state with a timestamp, removes the generated
