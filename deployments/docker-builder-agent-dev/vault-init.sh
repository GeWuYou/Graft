...
vault write pki/roles/graft-docker-builder-agent allowed_uri_sans='spiffe://graft/runtime-target/*/builder-agent/*' allow_glob_domains=true require_cn=false use_csr_common_name=false max_ttl=24h
...
printf '%s\n' 'path "pki/sign/graft-docker-builder-agent" { capabilities = ["update"] }' 'path "pki/cert/ca" { capabilities = ["read"] }' | vault policy write graft-docker-builder-agent -
...
vault read -field=role_id auth/approle/role/graft-docker-builder-agent/role-id > "$root/secrets/role_id"
