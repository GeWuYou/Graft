...
func (r testRBACRepository) ListUserIDsByPermissionCode(context.Context, string) ([]uint64, error) {
	return nil, nil
}
...
func TestRegisterRegistersReadManagementContracts(t *testing.T) {
	...
	if menus[0].Path != "/access-control" || menus[0].TitleKey != rbaccontract.AccessControlMenuTitle.String() {
		t.Fatalf("unexpected root menu: %#v", menus[0])
	}
	if menus[1].Path != "/access-control/overview" || menus[1].TitleKey != rbaccontract.AccessControlOverviewMenuTitle.String() {
		t.Fatalf("unexpected overview menu: %#v", menus[1])
	}
	if menus[2].Path != "/access-control/roles" || menus[2].Permission != rbaccontract.RoleReadPermission.String() {
		t.Fatalf("unexpected role menu: %#v", menus[2])
	}
	if menus[3].Path != "/access-control/permissions" || menus[3].Permission != rbaccontract.PermissionReadPermission.String() {
