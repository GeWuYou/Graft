...
`server/modules/audit` 提供当前 MVP 阶段的最小审计插件，用于把请求级自动审计和业务主动审计收敛为稳定落盘行为。
...
* 写入逻辑复用 `server/internal/audit`、`server/modules/audit/store` 和插件自有 `storeent` SQL repository
