...
type authFlowBridge struct {
	auth      *authService
	bootstrap moduleapi.UserBootstrapProvider
}
...
func (b authFlowBridge) ReadBootstrapPayload(ctx context.Context, request *http.Request) (moduleapi.AuthBootstrapPayload, error) {
	payload, err := b.bootstrap.ReadBootstrap(ctx, request)
	if err != nil {
		return moduleapi.AuthBootstrapPayload{}, err
	}
	credential, err := b.auth.currentUserCredential(ctx, payload.User.Username)
	if err != nil {
		return moduleapi.AuthBootstrapPayload{}, err
	}
	payload.MustChangePassword = credential.MustChangePassword
	return payload, nil
}
