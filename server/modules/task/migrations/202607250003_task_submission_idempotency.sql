...
ALTER TABLE tasks
  ADD CONSTRAINT tasks_submission_fingerprint_sha256_check
    CHECK (submission_fingerprint IS NULL OR submission_fingerprint ~ '^[0-9a-f]{64}$') NOT VALID;

ALTER TABLE tasks
  VALIDATE CONSTRAINT tasks_idempotency_key_hash_sha256_check;

ALTER TABLE tasks
  VALIDATE CONSTRAINT tasks_submission_fingerprint_sha256_check;

CREATE UNIQUE INDEX CONCURRENTLY uq_tasks_idempotency_submission
