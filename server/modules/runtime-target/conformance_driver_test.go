...
func TestDockerRuntimeAgentFixtureTargetAuthorityRejectsUnavailableRepository(t *testing.T) {
	if _, err := (dockerRuntimeAgentFixtureTargetAuthority{}).Resolve(context.Background()); err == nil {
		t.Fatal("fixture target authority accepted an unavailable repository")
	}
}

func TestWriteDockerRuntimeAgentConformanceMaterialRestrictsPermissions(t *testing.T) {
	path := filepath.Join(t.TempDir(), "secrets", "bootstrap.json")
	if err := writeDockerRuntimeAgentConformanceMaterial(path, DockerRuntimeAgentConformanceMaterial{TargetID: 1, AgentID: "builder", BootstrapToken: "secret"}); err != nil {
...
func TestConformancePayloadFingerprintIsStableAndScoped(t *testing.T) {
	scenario := DockerRuntimeAgentFixtureScenario{AgentID: "builder", ImageDigest: "sha256:fixture", AgentVersion: "v1", EnrollmentRef: "fixture"}
