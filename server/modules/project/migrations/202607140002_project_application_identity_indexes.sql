...
ALTER TABLE compose_projects
  VALIDATE CONSTRAINT compose_projects_application_name_format_check;
ALTER TABLE compose_projects
...
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS compose_projects_workspace_path_live
  ...
  WHERE deleted_at = 0;
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS compose_projects_application_name_live
  ON compose_projects (application_name)
  WHERE deleted_at = 0 AND application_name IS NOT NULL;
