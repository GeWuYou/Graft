...
func (s *Service) SourceCatalog(ctx context.Context) (SourceCatalogResult, error) {
	...
	items := []generated.ProjectSourceEntry{
		...
		},
		{
			...
			Permission:      projectcontract.ProjectCreatePermission.String(),
			MenuGroup:       projectcontract.ProjectMenuPath,
			Description:     "Create a managed project from a credential-free Git checkout staged outside the managed root.",
			...
			StatusReason:    stringPointer("Public and locally reachable repositories are supported. Credentials are not accepted or stored."),
			StatusReasonKey: stringPointer("project.createSource.statusReason.gitReady"),
		},
		{
