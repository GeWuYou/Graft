...
// Package main 提供仅供 docker-runtime-agent fixture 使用的 conformance 进程入口。
package main
...
func main() {
	if err := run(context.Background(), os.Args[1:], os.Stdout); err != nil {
		fmt.Fprintln(os.Stderr, "docker-runtime-agent conformance failed:", err)
		os.Exit(1)
	}
}
...
func run(ctx context.Context, args []string, output *os.File) error {
	...
	var evidence runtimetarget.DockerRuntimeAgentConformanceEvidence
	switch phase {
	case "prepare":
...
func newDriver(db *sql.DB, cfg *config.Config) (*runtimetarget.DockerRuntimeAgentConformanceDriver, error) {
	if cfg == nil {
	...
	return runtimetarget.NewDockerRuntimeAgentConformanceDriver(db, pepper, moduleapi.AgentCertificateIssuer(issuer))
}
...
func parseArguments(args []string) (string, runtimetarget.DockerRuntimeAgentFixtureScenario, verificationBaseline, error) {
	if len(args) > 0 && args[0] != "" && args[0][0] != '-' {
		args = append([]string{"--phase", args[0]}, args[1:]...)
	}
	flags := flag.NewFlagSet("graft-docker-runtime-conformance", flag.ContinueOnError)
	flags.SetOutput(os.Stderr)
	phase := flags.String("phase", "", "prepare, verify-bootstrap, or verify-restart")
	agentID := flags.String("agent-id", envOr("GRAFT_CONFORMANCE_AGENT_ID", "docker-runtime-agent"), "fixture agent identity")
	...
	enrollmentRef := flags.String("enrollment-ref", envOr("GRAFT_CONFORMANCE_ENROLLMENT_REF", "docker-runtime-agent-conformance"), "fixture enrollment reference")
	automationID := flags.String("automation-id", envOr("GRAFT_CONFORMANCE_AUTOMATION_ID", "docker-runtime-agent-conformance"), "fixture delivery actor")
	dockerInstallationRef := flags.String("docker-installation-ref", envOr("GRAFT_CONFORMANCE_DOCKER_INSTALLATION_REF", "docker-runtime-agent-conformance"), "fixture Docker installation reference")
	dockerSecretRef := flags.String("docker-secret-ref", envOr("GRAFT_CONFORMANCE_DOCKER_SECRET_REF", "docker-runtime-agent-bootstrap"), "fixture Docker secret reference")
	...
	if err := flags.Parse(args); err != nil {
		return "", runtimetarget.DockerRuntimeAgentFixtureScenario{}, verificationBaseline{}, err
	}
	...
	if *phase != "prepare" && *phase != "verify-bootstrap" && *phase != "verify-restart" {
		return "", runtimetarget.DockerRuntimeAgentFixtureScenario{}, verificationBaseline{}, errors.New("phase must be prepare, verify-bootstrap, or verify-restart")
	}
	if scenario.AgentID == "" {
		return "", runtimetarget.DockerRuntimeAgentFixtureScenario{}, verificationBaseline{}, errors.New("agent-id is required")
	}
	if *phase == "prepare" && (scenario.ImageDigest == "" || scenario.AgentVersion == "" || scenario.EnrollmentRef == "" || scenario.ExpectedAutomationID == "" || scenario.DockerInstallationRef == "" || scenario.DockerSecretRef == "" || scenario.BootstrapMaterialFile == "" || scenario.AgentConfigFile == "" || scenario.BootstrapURL == "" || scenario.AgentURL == "" || scenario.BootstrapCAFile == "" || scenario.TrustBundleFile == "" || scenario.AgentSecretFile == "") {
		return "", runtimetarget.DockerRuntimeAgentFixtureScenario{}, verificationBaseline{}, errors.New("prepare requires fixture scenario fields")
	}
	if *phase == "verify-bootstrap" && baseline.targetID != 0 && baseline.targetID < 1 {
		return "", runtimetarget.DockerRuntimeAgentFixtureScenario{}, verificationBaseline{}, errors.New("verify-bootstrap requires target-id")
	}
	if *phase == "verify-restart" && baseline.targetID != 0 && (baseline.identityID == "" || baseline.generation < 1 || baseline.receiptCount < 1) {
		return "", runtimetarget.DockerRuntimeAgentFixtureScenario{}, verificationBaseline{}, errors.New("verify-restart requires bootstrap evidence")
	}
...
func validateRestartEvidence(baseline verificationBaseline, evidence runtimetarget.DockerRuntimeAgentConformanceEvidence) error {
...
func writeVerificationBaseline(path string, evidence runtimetarget.DockerRuntimeAgentConformanceEvidence) error {
