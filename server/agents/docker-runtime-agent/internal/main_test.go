...
func TestStableSPIFFEIdentityExcludesGeneration(t *testing.T) {
	if got := stableSPIFFE(7, "builder-1"); got != "spiffe://graft/runtime-target/7/runtime-agent/builder-1" {
		t.Fatalf("identity = %s", got)
	}
}

func TestDefaultConfigFileUsesExplicitEnvironment(t *testing.T) {
	t.Setenv(configPathEnvironment, "/tmp/docker-runtime-agent.json")
	if got := defaultConfigFile(); got != "/tmp/docker-runtime-agent.json" {
