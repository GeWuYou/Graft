...
官方根 Compose 的 `docker-builder-agent` profile 使用与 server/web 相同的 `GRAFT_IMAGE_TAG`。profile 只挂载配置、token、trust bundle、state 和 Docker socket；启动前必须完成 Backend 的 Vault PKI、bootstrap TLS、mTLS 与 delivery 准备，挂载契约以 [Agent Protocol](../../../ai-plan/design/architecture/credential-vault-and-runtime-target-agent-protocol.md) 为准。
